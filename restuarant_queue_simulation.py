...
#Read csv file to calculate arrival interaval time and avg service time
df = pd.read_csv("dataset.csv")

# calculations
df["waiting_time"] = df["Service_Start_Time"] - df["Arrival_Time"]
df["service_time"] = df["Service_End_Time"] - df["Service_Start_Time"]

total_time = df["Service_End_Time"].max()
num_customers = len(df)

arrival_interval_real = total_time / num_customers
avg_service_time_real = df["service_time"].mean()
...
if __name__ == "__main__":
    print("\n ~~~ FAST FOOD RESTUARANT QUEUE SIMULATION ~~~\n")

    print(" \nParameters calculated from real data ")
    print(f"1) Arrival interval : {arrival_interval_real:.2f} min")
    print(f"2) Average service time : {avg_service_time_real:.2f} min")
    ...
    results.append(run_simulation(
        num_cashiers=2, 
        arrival_interval=arrival_interval_real, 
        avg_service_time=avg_service_time_real, 
    ...
    results.append(run_simulation(
        ...
        arrival_interval=arrival_interval_real, 
        avg_service_time=avg_service_time_real, 
        scenario_name= "More Cashier"))

    # Scenario 3 - peak time
    results.append(run_simulation(
        num_cashiers=2, 
        arrival_interval=arrival_interval_real*0.7, 
        avg_service_time=avg_service_time_real, 
        scenario_name= "Peak Time"))
    ...
    results.append(run_simulation(
        ...
        arrival_interval=arrival_interval_real*0.7, 
        avg_service_time=avg_service_time_real, 
        scenario_name= "Peak & More Cashiers"))

    # Create Summary Table
    print("\n" )
    print("SCENERIO EXPERIMENT SUMMARY TABLE")
    print("-"*94)
